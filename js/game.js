...
function differenceBetweenAngles(angleA, angleB)
{
    var diffBetween = threeSixtyFy(angleA) - threeSixtyFy(angleB);
    if(diffBetween > 180)
    {
...
{
    var magnitude = Math.abs(startingMagnitude);
    if(startingMagnitude < 0)
    {
        startingAngle += 180;
    }
    var angle = threeSixtyFy(startingAngle);
    ...
    {
        angle = threeSixtyFy(newAngle);
    };
    this.setMagnitude = function(newMagnitude)
    {
        magnitude = Math.abs(newMagnitude);
        if(newMagnitude < 0)
        {
            angle = threeSixtyFy(angle + 180);
        }
    };
    this.setXComponent = function(newXComp)
    {
        var yComp = this.getYComponent();
        magnitude = Math.sqrt((newXComp * newXComp) + (yComp * yComp));
        angle = toDegrees(Math.atan(yComp / newXComp));
        if(newXComp < 0)
        {
            angle += 180;
        }
        angle = threeSixtyFy(angle);
    };
    this.setYComponent = function(newYComp)
    {
        var xComp = this.getXComponent();
        magnitude = Math.sqrt((newYComp * newYComp) + (xComp * xComp));
        angle = toDegrees(Math.atan(xComp / newYComp));
        if(xComp < 0)
        {
            angle += 180;
        }
    ...
    {
        this.setMagnitude(magnitude * -1);
    };
}

Vector.sum = function(vectorA, vectorB)
{
    var sumVec = new Vector(1,0);
    sumVec.setXComponent(vectorA.getXComponent() + vectorB.getXComponent());
    sumVec.setYComponent(vectorA.getYComponent() + vectorB.getYComponent());
    return sumVec;
};

Vector.difference = function(minuendVector, subtrahendVector)
{
    var difVec = new Vector(1,0);
    difVec.setXComponent(minuendVector.getXComponent() - subtrahendVector.getXComponent());
    difVec.setYComponent(minuendVector.getYComponent() - subtrahendVector.getYComponent());
    return difVec;
};

Vector.dot = function(vectorA, vectorB)
{
    var diffBetween = differenceBetweenAngles(vectorA.getAngle(), vectorB.getAngle());
    if(Math.abs(diffBetween) === 90)
    {
        return 0;
    }
    else
    {
        return vectorA.getMagnitude() * vectorB.getMagnitude() * Math.cos(toRadians(diffBetween));
    }
};

Vector.projection = function(projOfVector, ontoVector)
{
    var outVec = new Vector(0, ontoVector.getAngle());
    if(ontoVector.getMagnitude() !== 0)
    {
        outVec.setMagnitude(Vector.dot(projOfVector, new Vector(1,ontoVector.getAngle())));
    }
    return outVec;
};
...
/**
 *
 * @param {type} startingX
 * @param {type} startingY
 * @param {type} minimumX
 * @param {type} minimumY
 * @param {type} maximumX
 * @param {type} maximumY
 * @param {type} dotRadius
 * @param {type} bounceEffeciency
 * @param {type} acceleration
 * @param {type} maximumSpeed
 * @returns {MovingTarget}
 */
 ...
 * @param {type} gunBaseRadius
 * @param {type} gunTipRadius
 * @param {type} gunWidthAngle
 * @param {type} minimumAngAcceleration
 * @param {type} maximumAngularAcceleration
 * @param {type} shotsPerSecond
 * @param {type} degreesError
 * @returns {Turret}
 * @constructor 
 */
...
var render = function(){
    ...
    //debug text
    var vecA = new Vector(10,0);
    var vecB = new Vector(10, 135);
    ctx.strokeText("VecA x: " + vecA.getXComponent(), 20, i += 20);
    ctx.strokeText("VecA y: " + vecA.getYComponent(), 20, i += 20);
    ctx.strokeText("VecB x: " + vecB.getXComponent(), 20, i += 20);
    ctx.strokeText("VecB y: " + vecB.getYComponent(), 20, i += 20);
    ctx.strokeText("A dot B: " + Vector.dot(vecA, vecB), 20, i += 20);
    ctx.strokeText("Projection magnitude: " + Vector.projection(vecB, vecA).getMagnitude(), 20, i += 20);
    ctx.strokeText("Projection angle: " + Vector.projection(vecB, vecA).getAngle(), 20, i += 20);
    
    ctx.strokeText("Target speed: " + targetDrone.getSpeed().toFixed(1), 20, i += 20);
    ctx.strokeText("Movement Angle: " + targetDrone.getVelocityAngle().toFixed(1), 20, i += 20);

    ctx.strokeText("Relative angle: " + turret.getRelativeVelocityAngle().toFixed(1), 20, i += 20);

    ctx.strokeText("Angular velocity: " + turret.getRelativeAngularVelocity().toFixed(1), 20, i += 20);

    ctx.strokeText("Distance to target: " + turret.getDistanceToTarget().toFixed(1), 20, i += 20);

    ctx.strokeText("Turret angle: " + turret.getAngle().toFixed(1), 20, i += 20);
    ctx.strokeText("Desired angle: " + turret.getDesiredAngle().toFixed(1), 20, i += 20);
